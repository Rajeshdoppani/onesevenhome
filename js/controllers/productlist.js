    ...
    .controller('ProductlistCtrl', ['$scope', '$http', '$routeParams', '$location', '$window', 'resturl', function ($scope, $http, $routeParams, $location, $window, resturl) {
        ...
        if (localStorage.loggedInUser != undefined) {
            ...
            if ($location.path().split("/")[1] == "vendorcategories") {
                //$scope.vendoradd=true;
                $http.get(resturl + "/vendor/" + $scope.loggedInuserId + "/categories/" + $routeParams.cid + "?" + "pageNumber=1&pageSize=12").then(function (resp) {
                    $scope.page = 1;
        ...
        $http.get(resturl + "/getAllCategories").then(function (resp) {
			...
			$scope.categorySub = $scope.categorySub.filter(function(item) {
            ...
            if ($location.path().split("/")[1] == "vendorcategories") {
                ...
                $http.get(resturl + "/" + $scope.routeurl + "?" + "pageNumber=1&pageSize=12").then(function (resp) {
                    $scope.page = 1;
                    $scope.pagingincrement = false;
                    $scope.pagingnormal = true;
                    $scope.dropshow = false;
        ...
        if ($location.path().split("/")[1] == "vendorcategories") {
            $scope.page = 1;
            $scope.PagingAct = function (page, pageSize, total) {
                $http.get(resturl + "/vendor/" + $scope.loggedInuserId + "/" + $scope.routeurl + "?" + "pageNumber=" + page + "&pageSize=12").then(function (resp) {
                    $scope.pagingincrement = true;
                    $scope.normalproduct = true;
                    $scope.pagingnormal = false;
                    $scope.dropdownhide = true;
            ...
            $scope.page = 1;
            $scope.PagingAct = function (page, pageSize, total) {
                $http.get(resturl + "/" + $scope.routeurl + "?" + "pageNumber=" + page + "&pageSize=12").then(function (resp) {
                    $scope.pagingincrement = true;
                    $scope.normalproduct = true;
                    $scope.pagingnormal = false;
                    $scope.dropdownhide = true;
        ...
        $scope.fil = function (subfil) {
            ...
            if ($location.path().split("/")[1] == "vendorcategories") {
                $http.get(resturl + "/vendor/" + $scope.loggedInuserId + "/" + $scope.routeurl + "?" + "pageNumber=1&pageSize=12").then(function (resp) {
                    $scope.routedata = resp;
                    $scope.plist = resp.data.responseData;
                    if (resp.data.responseData.length == "0") {
                        $scope.prodshow = true;
        ...
        myEndListener = function () {
            ...
            $scope.PagingAct = function (page, pageSize, total) {
                ...
                $http.post(resturl + "/getProductsByFiltersAndPrice" + "?" + "pageNumber=" + page + "&pageSize=12", priceRange).then(function (resp) {
                    $scope.plist = resp.data.filteredProducts;
                    if (resp.data.paginationData == null) {
                        $scope.totalCount = 0;
                        $scope.plist = "";
        ...
        $scope.pricing = function (priceValue) {
            ...
            $http.post(resturl + "/getProductsByFiltersAndPrice" + "?" + "pageNumber=1&pageSize=12", priceRange).then(function (resp) {
                if (resp.data.filteredProducts.length == "0") {
                    $scope.filtershow = true;
                    $scope.pagingnormal = false;
                    $scope.normalproduct = false;
