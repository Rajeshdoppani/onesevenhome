angular.module('newapp')
	.controller('DashboardCtrl', ['$scope', '$http', '$location', '$routeParams', '$filter', 'resturl', '$interval', '$rootScope', function ($scope, $http, $location, $routeParams, $filter, resturl, $interval, $rootScope) {
		...
		$http.get(resturl + "/getDealOfDay").then(function (resp) {
			...
			if (resp.data.dealOfDay != null) {
				var dd = new Date(resp.data.dealOfDay.productPriceSpecialEndDate);
		...
		$http.get("js/controllers/whatwedo.json").then(function (resp) {
			...
			$scope.whatWeDoLoaded = true;
			$scope.whatWeDoConfig = {
				dots: false,
				arrows: true,
				infinite: true,
				slidesToShow: 5,
		...
		// 	};
		// });
$http.get("js/controllers/subcatimages.json").then(function (resp) {
			$scope.catImageData = resp.data;
